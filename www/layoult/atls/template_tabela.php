...
<?php }else if($params['Tipo']==='Dinamica'){
        if ($params['Opcao']['Tabela']){
            ...
            $contador = 0;  ?>
            <table class="table table-hover table-striped table-bordered dt-responsive Listagem_Table<?php if($params['Opcao']['Apagado1']){ ?> apagado1<?php } ?>" ordenar="[<?php echo $params['Opcao']['aaSorting']; ?>]"><thead><tr>
...
<?php }else if($params['Tipo']==='Massiva'){
        if ($params['Opcao']['Tabela']){
            $colunas = count($params['Opcao']['Tabela']);
            $contador = 0;  ?>
            <table url="<?php echo $params['Opcao']['Url']; ?>" class="table table-hover table-striped table-bordered dt-responsive datatable row<?php if($params['Opcao']['Apagado1']){ ?> apagado1<?php } ?>" ordenar="[<?php echo $params['Opcao']['aaSorting']; ?>]"><thead><tr>
