    ...
    class Mailer
    {
        ...
        protected function addAnAddress($kind, $address, $name = '')
        {
            if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
                ...
                if ($this->exceptions) {
                    throw new \Framework\Classes\Mailer\phpmailerException('Invalid recipient array: ' . $kind);
                }
                return false;
            }
            ...
            if (!$this->validateAddress($address)) {
                ...
                if ($this->exceptions) {
                    throw new \Framework\Classes\Mailer\phpmailerException($this->lang('invalid_address') . ': ' . $address);
                }
                return false;
            }
            if ($kind != 'Reply-To') {
        ...
        public function preSend()
        {
            try {
                ...
                if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
                    throw new \Framework\Classes\Mailer\phpmailerException($this->lang('provide_address'), self::STOP_CRITICAL);
                }
                ...
                if (!$this->AllowEmpty and empty($this->Body)) {
                    throw new \Framework\Classes\Mailer\phpmailerException($this->lang('empty_message'), self::STOP_CRITICAL);
                }
            ...
            if ($this->SingleTo === true) {
                foreach ($this->SingleToArray as $val) {
                    if (!@$mail = popen($sendmail, 'w')) {
                        throw new \Framework\Classes\Mailer\phpmailerException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                    }
                    fputs($mail, "To: " . $val . "\n");
                    fputs($mail, $header);
                    fputs($mail, $body);
        ...
        protected function mailSend($header, $body)
        {
            ...
            if (!$rt) {
                throw new \Framework\Classes\Mailer\phpmailerException($this->lang('instantiate'), self::STOP_CRITICAL);
            }
            return true;
        }
        ...
        public function getSMTPInstance()
        {
            if (!is_object($this->smtp)) {
                $this->smtp = new \Framework\Classes\Mailer\SMTP;
            }
            return $this->smtp;
        }
        ...
        protected function smtpSend($header, $body)
        {
            ...
            if (!$this->smtpConnect()) {
                throw new \Framework\Classes\Mailer\phpmailerException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
            }
            ...
            if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
                throw new \Framework\Classes\Mailer\phpmailerException($this->lang('data_not_accepted'), self::STOP_CRITICAL);
            }
            ...
            if (count($bad_rcpt) > 0) { //Create error message for any bad addresses
                throw new \Framework\Classes\Mailer\phpmailerException(
                    ...
                    try {
                        ...
                        if ($tls) {
                            if (!$this->smtp->startTLS()) {
                                throw new \Framework\Classes\Mailer\phpmailerException($this->lang('connect_host'));
                            }
                        ...
                        if ($this->SMTPAuth) {
                            if (!$this->smtp->authenticate(
                                ...
                                throw new \Framework\Classes\Mailer\phpmailerException($this->lang('authenticate'));
                            }
                        }
                        return true;
                    } catch (phpmailerException $e) {
            ...
            if ($this->isError()) {
                ...
                try {
                    if (!defined('PKCS7_TEXT')) {
                        throw new \Framework\Classes\Mailer\phpmailerException($this->lang('signing') . ' OpenSSL extension missing.');
                    }
                    ...
                    if (@openssl_pkcs7_sign(
                        ...
                        @unlink($file);
                        @unlink($signed);
                        throw new \Framework\Classes\Mailer\phpmailerException($this->lang('signing') . openssl_error_string());
                    }
                } catch (phpmailerException $e) {
                    $body = '';
                    if ($this->exceptions) {
        ...
        public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
        {
            try {
                if (!@is_file($path)) {
                    throw new \Framework\Classes\Mailer\phpmailerException($this->lang('file_access') . $path, self::STOP_CONTINUE);
                }
        ...
        protected function encodeFile($path, $encoding = 'base64')
        {
            try {
                if (!is_readable($path)) {
                    throw new \Framework\Classes\Mailer\phpmailerException($this->lang('file_open') . $path, self::STOP_CONTINUE);
                }
        ...
        public function html2text($html, $advanced = false)
        {
            if ($advanced) {
                ...
                $h = new \Framework\Classes\Mailer\html2text($html);
                return $h->get_text();
            }
        ...
        public function set($name, $value = '')
        {
            try {
                if (isset($this->$name)) {
                    ...
                    throw new \Framework\Classes\Mailer\phpmailerException($this->lang('variable_set') . $name, self::STOP_CRITICAL);
                }
            } catch (Exception $e) {
                $this->setError($e->getMessage());
