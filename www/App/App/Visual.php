...
class Visual
{
    ...
    */
    ...
    private function Arquivos_Js_Get() {
        ...
        // Cria Cache
        $this->Javascript_Executar('DataSession.del(\'Dependencias_Js\');'
                . 'setTimeout(function() {DataSession.save(\'Dependencias_Js\',\''.implode('|', $this->arquivos_js).'\');},10);');
    ...
    private function Arquivos_Css_Get() {
        ...
        // Cria Cache
        $this->Javascript_Executar('DataSession.del(\'Dependencias_Css\');'
                . 'setTimeout(function() {DataSession.save(\'Dependencias_Css\',\''.implode('|', $this->arquivos_css).'\');},10);');
    ...
    private function Sistema_Extras() {
        ...
        $array_js = Array(
            ...
            'globals/scripts/sitec/Json/Tratar',
            'globals/scripts/sitec/Elements/Popup',
            'globals/scripts/sitec/Navigation/Call',
            'globals/scripts/sitec/Navigation/Url',
            'globals/scripts/sitec/Data/Cache',
            'globals/scripts/sitec/Data/Cookie',
            'globals/scripts/sitec/Data/Session',
         ...
         '$(\'#'.$idcalendar.'\').fullCalendar({
            ...
            eventClick: function(event) {
                if (UserLogado==0) {
                    ...
                    NavigationCall.init(\''.SISTEMA_MODULO.'/'.SISTEMA_SUB.'/Popup_Agendar_veiculo/\'+event.id+\'&nome=\'+event.title+\'&inicial=\'+event.comeco+\'&final=\'+event.final,\'\',\'GET\',true);	
                }
                return false;
            }
        });');
              ...
              foreach ($array as &$valor) {
                ...
                $table->addcorpo(array(
                    ...
                    array("nome" => $this->Tema_Elementos_Btn('Editar'     ,Array('Editar Categoria'        ,'categoria/Admin/Categorias_Edit/'.$valor['id'].'/'    , '')).
                                    $this->Tema_Elementos_Btn('Deletar'    ,Array('Deletar Categoria'       ,'categoria/Admin/Categorias_Del/'.$valor['id'].'/'     , __('Deseja realmente deletar essa Categoria ?')))
                    )
                ));
                ++$i;
