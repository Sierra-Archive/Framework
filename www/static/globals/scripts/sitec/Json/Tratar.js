var JsonTratar = function (Link, Json, History) {
    ...
    function Control_Ajax_Popup (json) {
        ...
        for(; i<tam; ++i) {
            if (json['botoes'][i]['clique'] === '$( this ).dialog( "close" );') {
                footer += '<button class="btn" data-dismiss="modal" aria-hidden="true" onCLick="ElementsPopup.Control_Ajax_Popup_Fechar(\''+json["id"]+'\');">'+json['botoes'][i]['text']+'</button>';
            } else {
    ...
    function Control_Ajax_Redirect (json) {
        console.log('JsonTratar -> Redirect');
        var cod         = '',
            script      = '',
            url         = [];
    ...
    function Control_Ajax_Css (json) {
        var script      = '',
            cache = DataSession.read('Dependencias_Css');
        ...
        if (script !== '') {
            // Salva Cache
            DataSession.save('Dependencias_Css',cache.join('|'));
    ...
    function Control_Ajax_Javascript (json) {
        var script = '',
            cache = DataSession.read('Dependencias_Js');
        ...
        console.log('JS',json,script);
        if(script!=='') {
            // Salva Cache
            DataSession.save('Dependencias_Js',cache.join('|'));
    ...
    function Modelo_Ajax_JsonTratar(url, data, navegador) {
        ...
        if (data !== null && typeof(data) === "object") {
            ...
            eval(cod);
            Sierra.Control_Layoult_Recarrega();
