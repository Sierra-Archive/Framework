<?php
class usuario_social_PersonaControle extends usuario_social_Controle
{
    /**
    * Construtor
    * 
    ...
    */
    ...
    protected static function Endereco_Persona($true=true){
        ...
        if($true===true){
            $_Controle->Tema_Endereco('Personas','usuario_social/Persona/Personas');
        }else{
    ...
    protected static function Endereco_Persona_Ver($persona,$true=true){
        ...
        if($true===true){
            $_Controle->Tema_Endereco($persona->nome,'usuario_social/Persona/Personas_View/'.$persona->id);
        }else{
    ...
    protected static function Endereco_Persona_Ver_Ficar_Ver($persona,$ficada,$true=true){
        ...
        if($true===true){
            $_Controle->Tema_Endereco($ficada->nome,'usuario_social/Persona/Personas_View/'.$persona->id);
        }else{
    ...
    public function Personas($export=false){
        ...
        $this->_Visual->Blocar($this->_Visual->Tema_Elementos_Btn('Superior'     ,Array(
            Array(
                'Adicionar Pessoa',
                'usuario_social/Persona/Personas_Add',
                ''
            ),
            Array(
                'Print'     => true,
        ...
        $personas = $this->_Modelo->db->Sql_Select('Usuario_Social');
    ...
    public function Personas_Add(){
        ...
        $formid     = 'SierraForm_usuario_social_Persona_Personas';
        $formbt     = 'Salvar';
        $formlink   = 'usuario_social/Persona/Personas_Add2/';
        $campos = Usuario_Social_DAO::Get_Colunas();
    ...
    public function Personas_Edit($id){
        ...
        $formlink   = 'usuario_social/Persona/Personas_Edit2/'.$id;
        $editar     = Array('Usuario_Social',$id);
        $campos = Usuario_Social_DAO::Get_Colunas();
    ...
    public function Personas_Del($id){
    	...
    	$id = (int) $id;
        // Puxa setor e deleta
        $setor = $this->_Modelo->db->Sql_Select('Usuario_Social', Array('id'=>$id));
    ...
    public function Personas_View($persona_id = false){
        ...
        usuario_social_AcaoControle::Acao_Stat($persona_id,'Esquerda');
        // Titulo
    ...
    public function Personas_Comentario($persona_id = false,$tipo='Unico'){
        ...
        $i = 0;
        $this->_Visual->Blocar('<a title="Adicionar Comentário de Persona" class="btn btn-success lajax explicar-titulo" acao="" href="'.URL_PATH.'usuario_social/Persona/Personas_Comentario_Add/'.$persona_id.'">Adicionar novo comentário nesse Persona</a><div class="space15"></div>');
        $comentario = $this->_Modelo->db->Sql_Select('Usuario_Social_Comentario',$where);
    ...
    public function Personas_Comentario_Add($persona_id = false){
        ...
        $persona = $this->_Modelo->db->Sql_Select('Usuario_Social',Array('id'=>$persona_id), 1);
        ...
        $formbt     = 'Salvar';
        $formlink   = 'usuario_social/Persona/Personas_Comentario_Add2/'.$persona_id;
        $campos = Usuario_Social_Comentario_DAO::Get_Colunas();
    ...
    public function Personas_Comentario_Edit($persona_id = false,$id = 0){
        ...
        $formlink   = 'usuario_social/Persona/Personas_Comentario_Edit2/'.$persona_id.'/'.$id;
        $editar     = Array('Usuario_Social_Comentario',$id);
        $campos = Usuario_Social_Comentario_DAO::Get_Colunas();
        self::DAO_Campos_Retira($campos, 'persona');
        \Framework\App\Controle::Gerador_Formulario_Janela($titulo1,$titulo2,$formlink,$formid,$formbt,$campos,$editar);
    }
    ...
    public function Personas_Comentario_Edit2($persona_id = false,$id = 0){
        ...
        $dao        = Array('Usuario_Social_Comentario',$id);
        $funcao     = '$this->Personas_View('.$persona_id.');';
    ...
    public function Ficou($persona_id=false,$tipo='Unico'){
        $i = 0;
        $this->_Visual->Blocar('<a title="Adicionar Nova Ficada" class="btn btn-success lajax explicar-titulo" acao="" href="'.URL_PATH.'usuario_social/Persona/Ficou_Add/'.$persona_id.'">Adicionar Nova Ficada</a><div class="space15"></div>');
        if($persona_id===false){
        ...
        $personas = $this->_Modelo->db->Sql_Select('Usuario_Social_Ficou',$where);
        if($personas!==false && !empty($personas)){
            if(is_object($personas)) $personas = Array(0=>$personas);
            reset($personas);
            foreach ($personas as $indice=>&$valor) {
                ...
                $tabela['Funções'][$i]          = $this->_Visual->Tema_Elementos_Btn('Zoom'       ,Array('Visualizar Ficada de Pessoa'        ,'usuario_social/Persona/Ficou_View/'.$valor->id.'/'.$persona_id  ,'')).
                                                  $this->_Visual->Tema_Elementos_Btn('Editar'     ,Array('Editar Ficada de Pessoa'        ,'usuario_social/Persona/Ficou_Edit/'.$valor->id.'/'.$persona_id      ,'')).
                                                  $this->_Visual->Tema_Elementos_Btn('Deletar'    ,Array('Deletar Ficada de Pessoa'       ,'usuario_social/Persona/Ficou_Del/'.$valor->id.'/'.$persona_id       ,'Deseja realmente deletar essa Ficada de Pessoa ?'));
                ++$i;
            }
    ...
    public function Ficou_Add($persona_id=false){
        ...
        $formid     = 'SierraForm_usuario_social_Persona_Ficou';
        $formbt     = 'Salvar';
        $campos = Usuario_Social_Ficou_DAO::Get_Colunas();
        if($persona_id!==false){
            self::DAO_Campos_Retira($campos,'persona1');
            $formlink   = 'usuario_social/Persona/Ficou_Add2/'.$persona_id;
        }else{  
            $formlink   = 'usuario_social/Persona/Ficou_Add2/';
        }
        \Framework\App\Controle::Gerador_Formulario_Janela($titulo1,$titulo2,$formlink,$formid,$formbt,$campos);
    }
    /**
    ...
    public function Ficou_Edit($id,$persona_id=false){
        ...
        $formbt     = 'Alterar Pessoa';
        if($persona_id!==false){
            $formlink   = 'usuario_social/Persona/Ficou_Edit2/'.$id.'/'.$persona_id;
        }else{  
            $formlink   = 'usuario_social/Persona/Ficou_Edit2/'.$id.'/';
        }
        $editar     = Array('Usuario_Social_Ficou',$id);
        $campos = Usuario_Social_Ficou_DAO::Get_Colunas();
        \Framework\App\Controle::Gerador_Formulario_Janela($titulo1,$titulo2,$formlink,$formid,$formbt,$campos,$editar);
    }
    /**
    ...
    public function Ficou_Edit2($id,$persona_id=false){
        ...
        $dao        = Array('Usuario_Social_Ficou',$id);
        if($persona_id===false){
            $funcao     = '$this->Ficou();';
        }else{
            $funcao     = '$this->Personas_View('.$persona_id.');';
    ...
    public function Ficou_Del($id,$persona_id=false){
    	...
    	$id = (int) $id;
        // Puxa setor e deleta
        $setor = $this->_Modelo->db->Sql_Select('Usuario_Social_Ficou', Array('id'=>$id));
        $sucesso =  $this->_Modelo->db->Sql_Delete($setor);
        // Mensagem
    	if($sucesso===true){
            $mensagens = array(
    ...
    public function Ficou_Comentario($ficada_id = false, $export = false){
        ...
        $this->_Visual->Blocar($this->_Visual->Tema_Elementos_Btn('Superior'     ,Array(
            Array(
                ...
                'usuario_social/Persona/Ficou_Comentario_Add/'.$ficada_id,
                ''
            ),
            Array(
                'Print'     => true,
                'Pdf'       => true,
                'Excel'     => true,
                'Link'      => 'usuario_social/Persona/Ficou_Comentario/'.$ficada_id,
            )
        )));
        $comentario = $this->_Modelo->db->Sql_Select('Usuario_Social_Ficou_Comentario',$where);
        if($comentario!==false && !empty($comentario)){
            if(is_object($comentario)) $comentario = Array(0=>$comentario);
            reset($comentario);
            foreach ($comentario as $indice=>&$valor) {
                ...
                $tabela['Data'][$i]         =   $valor->log_date_add;
                $tabela['Funções'][$i]      =   $this->_Visual->Tema_Elementos_Btn('Editar'          ,Array('Editar Comentário de Persona'        ,'usuario_social/Persona/Ficou_Comentario_Edit/'.$ficada_id.'/'.$valor->id.'/'    ,'')).
                                                $this->_Visual->Tema_Elementos_Btn('Deletar'         ,Array('Deletar Comentário de Persona'       ,'usuario_social/Persona/Ficou_Comentario_Del/'.$ficada_id.'/'.$valor->id.'/'     ,'Deseja realmente deletar esse Comentário?'));
                ++$i;
            }
            if($export!==false){
    ...
    public function Ficou_Comentario_Add($ficada_id = false){
        ...
        $ficada = $this->_Modelo->db->Sql_Select('Usuario_Social_Ficou',Array('id'=>$ficada_id), 1);
        ...
        $formid     = 'SierraForm_usuario_social_Persona_Ficou_Comentario';
        $formbt     = 'Salvar';
        $formlink   = 'usuario_social/Persona/Ficou_Comentario_Add2/'.$ficada_id;
        $campos = Usuario_Social_Ficou_Comentario_DAO::Get_Colunas();
    ...
    public function Ficou_Comentario_Add2($ficada_id = false){
        ...
        $dao        = 'Usuario_Social_Ficou_Comentario';
    ...
    public function Ficou_Comentario_Edit($ficada_id = false,$id = 0){
        ...
        $formlink   = 'usuario_social/Persona/Ficou_Comentario_Edit2/'.$persona_id.'/'.$id;
        $editar     = Array('Usuario_Social_Ficou_Comentario',$id);
        $campos = Usuario_Social_Ficou_Comentario_DAO::Get_Colunas();
        self::DAO_Campos_Retira($campos, 'ficada');
        \Framework\App\Controle::Gerador_Formulario_Janela($titulo1,$titulo2,$formlink,$formid,$formbt,$campos,$editar);
    }
    ...
    public function Ficou_Comentario_Edit2($ficada_id = false,$id = 0){
        ...
        $dao        = Array('Usuario_Social_Ficou_Comentario',$id);
