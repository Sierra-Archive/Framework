...
class predial_SalaoModelo extends predial_Modelo
{
    /**
    * Construtor
    * 
    ...
    */
    ...
    public function retorna_Agendadatas(&$datas,$veiculo=0) {
        ...
        FROM '.MYSQL_USUARIO_VEICULO_ALUGUEL.' WHERE data_inicial>'.$datainicial.''.$where.' ORDER BY data_inicial'); //P.categoria
        while ($campo = $sql->fetch_object()) {
    ...
    public function agendamento_inserir($veiculo,$data_inicial,$data_final,$valor) {
        ...
        $sql = $this->db->query('SELECT id FROM '.MYSQL_USUARIO_VEICULO_ALUGUEL.' WHERE data_cadastro=\''.APP_HORA.'\' AND user=\''.\Framework\App\Acl::Usuario_GetID_Static().'\' ORDER BY id DESC LIMIT 1');
        while ($campo = $sql->fetch_object()) {
           $id = $campo->id;
        }
