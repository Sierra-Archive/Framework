            ...
            if (!empty($anexos)) {
                foreach ($anexos as &$valor) {
                    ...
                    if (file_exists($endereco)) {
                        ...
                        $tipo       =   $valor->ext;
                        $table[__('Nome')][$i]             = '<a href="'.URL_PATH.'usuario_mensagem/Suporte/Download/'.$valor->id.'/" border="1" class="lajax" data-acao="">'.$valor->nome.'</a>';
                        $table[__('Tamanho')][$i]          = $tamanho.' KB';
                        $table[__('Data')][$i]             = $valor->log_date_add;
                        $table[__('Download')][$i]          = $this->_Visual->Tema_Elementos_Btn('Baixar'     ,Array(__('Download de Arquivo')  ,'usuario_mensagem/Suporte/Download/'.$valor->id    , ''));
                        ++$i;
                    }
                }
            }
