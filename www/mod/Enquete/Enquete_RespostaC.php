    ...
    */
    ...
    static function Respostas_Tabela(&$respostas) {
        ...
        foreach ($respostas as &$valor) {
            ...
            $table[__('Votos')][$i]            = $valor->qnt_votos.' / '.$qnt_votos_totais;
            $table[__('Data Registrado')][$i]  = $valor->log_date_add;
            $table[__('Funções')][$i]          = $Visual->Tema_Elementos_Btn('Editar'     ,Array(__('Editar Resposta')        ,'Enquete/Resposta/Respostas_Edit/'.$valor->enquete.'/'.$valor->id.'/'    , ''), $permissionEdit).
                                              $Visual->Tema_Elementos_Btn('Deletar'    ,Array(__('Deletar Resposta')       ,'Enquete/Resposta/Respostas_Del/'.$valor->enquete.'/'.$valor->id.'/'     , __('Deseja realmente deletar essa Resposta ?')), $permissionDelete);
            ++$i;
        }
