...
class Transporte_EstradaControle extends Transporte_Controle
{
    ...
    static function Estradas_Tabela(&$estrada) {
        ...
        foreach ($estrada as &$valor) {                
            ...
            $table[__('Foto')][$i]         = '<img alt="'.__('Foto da Dica de Estrada').'" src="'.$valor->foto.'" style="max-width:100px;" />';
            $table[__('Estrada')][$i]       = $valor->nome;
            ...
            $table[__('Funções')][$i]      = '<span id="status'.$valor->id.'">'.$Visual->Tema_Elementos_Btn('Status'.$valor->status     ,Array($texto        ,'Transporte/Estrada/Status/'.$valor->id.'/'    , ''), $permissionStatus).'</span>';
            if ($valor->destaque==1) {
                $texto = __('Em Destaque');
            } else {
                $texto = __('Não está em destaque');
            }
            $table[__('Funções')][$i]      .= '<span id="destaques'.$valor->id.'">'.$Visual->Tema_Elementos_Btn('Destaque'.$valor->destaque   ,Array($texto   ,'Transporte/Estrada/Destaques/'.$valor->id.'/'    , ''), $permissionFeatured).'</span>';
            $table[__('Funções')][$i]      .= $Visual->Tema_Elementos_Btn('Editar'     ,Array(__('Editar Dica de Estrada')        ,'Transporte/Estrada/Estradas_Edit/'.$valor->id.'/'    , ''), $permissionEdit).
                                           $Visual->Tema_Elementos_Btn('Deletar'    ,Array(__('Deletar Dica de Estrada')       ,'Transporte/Estrada/Estradas_Del/'.$valor->id.'/'     , __('Deseja realmente deletar esse Dica de Estrada ?')), $permissionDelete);
            ++$i;
        }
